...
This vignette demonstrates how to use the `mitey` package to estimate
two key epidemiological parameters:

- **Serial interval:** The time between symptom onset in an infector and
  symptom onset in an infectee
- **Time-varying reproduction number (Rt):** The average number of
  secondary cases generated by each infected individual at time t

Understanding these parameters is crucial for characterizing disease
...
mathematical models that inform public health decision-making.

We’ll use both simulated and real-world examples for each parameter,
with a focus on scabies (a skin infestation caused by the mite Sarcoptes
scabiei) for the real-world applications.

## Estimating the Serial Interval

The serial interval distribution is crucial for understanding disease
transmission dynamics. The `mitey` package implements the method
developed by Vink et al. (2014)¹ to estimate the mean and standard
...
First, let’s generate simulated data with a known serial interval
...
![](quick_start_guide_files/figure-html/simulate-si-data-1.png)

In this simulation, we generate different types of transmission routes:

- **Co-Primary (CP):** Cases infected by the same source around the same
  time
- **Primary-Secondary (PS):** Direct transmission from index case to
  secondary cases
- **Primary-Tertiary (PT):** Unobserved secondary case between two cases
- **Primary-Quaternary (PQ):** Unobserved secondary and tertiary cases
  between two cases
...
The
[`si_estim()`](https://kylieainslie.github.io/mitey/reference/si_estim.md)
function uses a mixture model to distinguish between different
transmission routes. The output includes:
...
with minimal pre-symptomatic transmission.
...
![](quick_start_guide_files/figure-html/plot-sim-fit-1.png)
...
Now, let’s analyze real data from scabies outbreaks. We’ll use data from
...
Scabies has an unusually long serial interval compared to many other
...
typically have serial intervals of only 2-4 days.

We’ll use
[`si_estim()`](https://kylieainslie.github.io/mitey/reference/si_estim.md)
to estimate the serial interval for each study:
...
Estimated mean and standard deviation of serial interval (days) by study

The notable variation in estimated serial intervals between studies
...
context-specific estimates for epidemiological modeling.
...
![](quick_start_guide_files/figure-html/plot-si-multi-1.png)

These plots show considerable variation in estimated serial intervals
...
potential changes over time. The `mitey` package implements the
Wallinga-Lipsitch method for estimating Rt from time series of case
...
function with both simulated and real data.

#### Method details

The Wallinga-Lipsitch method² establishes a relationship between the
growth rate of an epidemic and the reproduction number. This approach:

- Uses the serial interval distribution to connect observed cases over
  time
- Calculates relative likelihoods of transmission between cases
- Adjusts for right-truncation (recent cases that haven’t had time to
  infect others)
- Produces estimates of the case reproduction number (Rt)

The
[`wallinga_lipsitch()`](https://kylieainslie.github.io/mitey/reference/wallinga_lipsitch.md)
function in `mitey` takes the following parameters:
...
Let’s generate epidemic data with a known time-varying reproduction
number to validate our implementation:
...
![](quick_start_guide_files/figure-html/sim-rt-1.png)

Note that the epidemic curve directly reflects changes in the
reproduction number:
...
Now, let’s estimate Rt using the
[`wallinga_lipsitch()`](https://kylieainslie.github.io/mitey/reference/wallinga_lipsitch.md)
function:
...
![](quick_start_guide_files/figure-html/estimate-rt-1.png)

Note that the estimated Rt (blue line) seems to lag behind the true Rt
(red line). This is expected because the case reproduction number we’re
estimating is based on when cases are observed, while the instantaneous
reproduction number (our true Rt) represents current transmission
potential. The choice of smoothing window is important - too small and
the estimates will be noisy, too large and they may miss important
...
The
[`wallinga_lipsitch()`](https://kylieainslie.github.io/mitey/reference/wallinga_lipsitch.md)
function includes a `shift` parameter that can help align case
reproduction number estimates with instantaneous reproduction number
estimates. When set to `TRUE`, this parameter shifts Rt estimates
forward by one mean serial interval to better match the timing of
...
![](quick_start_guide_files/figure-html/rt-shift-1.png)

The shifted estimates align better with the true Rt values. The shift
parameter moves estimates forward by one mean serial interval, which
helps address the inherent lag in case-based Rt estimation.
...
the `outbreaks` package³.

The Zika epidemic in Colombia was part of a larger outbreak across the
Americas in 2015-2016. Zika virus is primarily transmitted by Aedes
...
of microcephaly in newborns and Guillain-Barré syndrome⁴.
...
#>  $ cases: int  1 2 1 4 2 5 2 4 5 4 ...
```

Let’s have a look at the epidemic curve.
![](quick_start_guide_files/figure-html/zike_epicurve-1.png)

Now we’ll estimate the reproduction number using the
[`wallinga_lipsitch()`](https://kylieainslie.github.io/mitey/reference/wallinga_lipsitch.md)
function we created. For Zika virus, we’ll use a mean serial interval of
7 days with a standard deviation of 1.5 days, based on literature
estimates for mosquito-borne diseases:
...
![](quick_start_guide_files/figure-html/rt_viz-1.png)
...
- Around November-December, Rt crosses below 1, suggesting that control
  measures or other factors were beginning to reduce transmission
- By January, Rt remains below 1, suggesting the epidemic was being
  controlled

This analysis demonstrates how Rt estimation can provide valuable
...
Vink, M. A., Bootsma, M. C. & Wallinga, J. Serial intervals of
respiratory infectious diseases: A systematic review and analysis.
*American Journal of Epidemiology* **180**, 865–875 (2014).
...
of the Royal Society B: Biological Sciences* **274**, 599–604 (2007).

3\.

Kamvar, Z. N., Cai, J., Pulliam, J. R., Schumacher, J. & Jombart, T.
*[Outbreaks: A Collection of Disease Outbreak
Data](https://CRAN.R-project.org/package=outbreaks)*. (2019).

4\.

Rojas, D. P. *et al.* The epidemiology and transmissibility of zika
virus in girardot and san andres island, colombia, september 2015 to
january 2016. *Eurosurveillance* **21**, 30283 (2016).
