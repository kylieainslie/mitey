...
get_abs_prob <- function(dat){

  result_mat <- matrix(, nrow = 7, ncol = length(dat))

  for(i in 1:7){

    tmp_zero <- vec_int(dat, l = dat, u = (dat + 1), rt = i, q = "zero")
    tmp_lower <- vec_int(dat, l = (dat-1), u = dat, rt = i, q = "lower")
    tmp_upper <- vec_int(dat, l = dat, u = (dat + 1), rt = i, q = "upper")
