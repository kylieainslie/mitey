...
test_that("plot_si_fit rejects weights of wrong length", {
  ...
  expect_error(
    plot_si_fit(dat, mean = 10, sd = 3,
                weights = c(0.1, 0.7),   # length 2, but n_routes = 4 expects 3
                dist = "normal", n_routes = 4),
    regexp = "n_routes - 1"
  )
})
