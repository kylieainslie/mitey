...
#'
result_normal <- si_estim_flex(large_icc, dist = "normal",active_routes = c("PS", "PT","PQ"))
result_normal
...
si_estim_flex <- function(
  ...
  if (!"PS" %in% active_routes) {
    stop("La route 'PS' (Primary-Secondary) est obligatoire pour estimer les paramètres de la distribution.")
  }

  valid_routes <- c("CP", "PS", "PT", "PQ")
  if (!all(active_routes %in% valid_routes)) {
    stop("Routes invalides. Choisissez parmi : CP, PS, PT, PQ.")
  }
...
#ADD MAPPING FOR ACTIVE ROUTES
  ...
  comp_vec <- unlist(mapping[active_routes])
  comp_vec <- sort(comp_vec)

  # --- Dictionnaire des noms de composantes ---
  all_comp_names <- c(
    "1" = "CP",
    "2" = "PS_Forward", "3" = "PS_Backward",
    "4" = "PT_Forward", "5" = "PT_Backward",
    "6" = "PQ_Forward", "7" = "PQ_Backward"
  )

  # On extrait les noms correspondant aux composantes réellement calculées
  active_comp_names <- all_comp_names[as.character(comp_vec)]
  ...
  run_single_em <- function(mu_init, sigma_init) {
    ...
    for (k in 1:n) {
      ...
      w <- rowSums(tau) / j
      idx_ps <- which(comp_vec == 2)

      # Update parameters
      if (dist == "normal") {

        mu <- weighted.mean(dat, tau[idx_ps, ])

        sigma <- sqrt(weighted_var(dat, tau[idx_ps, ]))
      } else if (dist == "gamma") {
        opt <- optim(
          par = c(mu, sigma),
          wt_loglik,
  ...
  # Juste avant le return(best_result)
  ...
  names(best_result$wts) <- active_comp_names
  return(best_result)
}
