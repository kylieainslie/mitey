...
studies [@ariza2013; @kaburi2019; @tjon-kon-fat2021; @akunzirwe2023] of scabies outbreaks (Table 1<!--\@ref(tab:si_results_tab)-->), we
...
which we assume the serial intervals for scabies can be approximated by a Gamma distribution (see Appendix).
...
Bayesian hierarchical random-effects model in which we assume that there is a typical distribution for serial intervals, from which an outbreak-specific mean serial interval is sampled [@harrer]. We specified a prior distribution N(100,50) for the true pooled mean and Cauchy(0,1) for the between-study heterogeneity. We performed sensitivity analyses on our choices of prior distributions (see Appendix).
...
We performed a sensitivity analysis in which we estimated the mean and standard deviation of the serial interval assuming an underlying Gamma distribution (Table S1<!-- \@ref(tab:tab_si_gam --> and Figure S1<!--\@ref(fig:gam_si_plots) -->). We found that we obtained similar mean estimates as compared to assuming an underlying Normal distribution (albeit lower for most studies), but the estimated standard deviations were larger. Model fit was assessed visually by plotting the estimated densities over the epidemic curves (Figure S1<!--\@ref(fig:gam_si_plots) -->) and we found that the Gamma distribution was not a good fit of the data.
...
interval: 91.44, 153.41 days) (Figure 2<!--\@ref(fig:forest_plot-->). As we saw with the individual study estimates (Table 1<!--\@ref(tab:si_results_tab)-->),
...
distributions of each study's serial interval shown in Figure 2<!--
\@ref(forest_plot)-->). A normal distribution is assumed, and is
...
To determine the pattern of scabies spread over time, particularly if the incidence of scabies infections displays a repeating temporal pattern (e.g., seasonal waves), we first plotted weekly incidence of scabies infections per 1000 people in The Netherlands from 2011 to 2023 (Figure 4<!-- \@ref(fig:epidemic_curve)-->). We observed approximate yearly peaks and troughs of incidence with the amplitude of peak incidence gradually increasing over time. There were marked increases in peak incidence in 2022, 2023, and the beginning of 2024 (Figure 4<!--\@ref(fig:epidemic_curve)-->). When looking at cumulative incidence by month, we saw that incidence was typically highest in autumn (September - November) and winter (December - February), but in some years there were rises in incidence in spring (March - May) (Figure SX). Cumulative incidence was typically lowest in the summer months (June - August).

```{r epidemic_curve, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figure 4. Incidence of scabies infections per 1000 people in the Netherlands by date of diagnosis.", out.width="100%"}
knitr::include_graphics("figures/nivel_epidemic_curve.png")
```

Next, we estimated the time-varying case reproduction number of weekly scabies diagnoses (Figure 5<!--\@ref(fig:rt_plot)-->). As with weekly incidence, we see approximate yearly waves of transmission, with $R_t$ peaking in summer. The time difference in peak incidence (winter, Figure SX) versus peak reproduction number (summer) can be explained by the long serial interval of scabies (123 days with a 31-day standard deviation). Therefore, infections generated by cases in the winter are likely to manifest several months later, around summer, due to the long serial interval. Consequently, although the peak transmission events occur in winter, they influence the $R_t$ values seen in the following summer, aligning with the observed summer peaks in Figure 5. 
...
```{r rt_plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figure 5. Time-varying reproduction number of scabies transmission. Colored bands denote season. Winter = December 1 – February 28 (or 29 on leap year); Spring = March 1 – May 31; Summer = June 1 – August 31; Autumn = September 1 – November 31.", out.width="100%"}
...
Third, to estimate the time-varying reproduction number we use data on date of scabies diagnosis and assume that date of diagnosis is similar to date of symptom onset. However, this is a simplistic assumption and it is highly likely that symptom onset occurs some time before the date of diagnosis, causing our time-varying reproduction number estimates to be shifted to later dates. In a study of a scabies epidemic in Germany, the index case was not diagnosed for approximately 3 months following symptom onset [@ariza2013]. This delay in diagnosis can be due to the fact that scabies symptoms are not unique to scabies (itching, rash) or are misdiagnosed at first, thus causing a delay between symptom onset and diagnosis. While correcting for the delay between symptom onset and diagnosis of scabies cases would improve estimates of the time-varying reproduction number, this information is not routinely collected making correction for this delay difficult. Future studies should focus on trying to gather information about the delay between symptom onset and reporting. Additionally, the only estimate of the incubation period for scabies is the range provided by Mellanby of 4-6 weeks [@mellanby1944]. There is no data in the original study or in the literature to support this estimate or provide a point estimate with confidence bounds. Future research should aim to obtain more precise information on the incubation period of scabies infection.
