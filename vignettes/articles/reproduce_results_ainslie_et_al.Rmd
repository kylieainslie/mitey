...
```{r plot_growth_rate_model_fit, eval = FALSE, echo=FALSE}
# Define colors
data_color <- "#440154"      # Purple for observed data
model_color <- "#21908C"     # Teal for model fit

p <- ggplot(scabies_inc_total, aes(x = time)) +
  # Ribbon for confidence intervals
  geom_ribbon(aes(ymin = log(lower_fitted), ymax = log(upper_fitted)),
              fill = model_color, alpha = 0.2) +
  # Line for fitted values
  geom_line(aes(y = log(fitted), color = "Model Fit"), 
            linewidth = 1.2) +
  # Points for observed data
  geom_point(aes(y = log(cases), shape = "Observed Data"), 
             size = 3, color = data_color) +
  # Labels and theme
  labs(x = "Year", 
       y = "Annual Incidence per 1,000 (log scale)",
       color = NULL,
       shape = NULL) +
  scale_color_manual(values = c("Model Fit" = model_color)) +
...
ggsave(paste0(path_name,"scabies_incidence_model_fit.png"), 
       plot = p, 
       width = 8, 
       height = 5, 
       dpi = 300, 
