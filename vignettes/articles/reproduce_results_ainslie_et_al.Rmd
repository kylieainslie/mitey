...
rt_scabies <- wallinga_lipsitch(
  ...
  si_sd = scabies_si_sd,
  si_dist = "normal",  # Normal distribution for serial interval
  smoothing = 42,      # 6-week smoothing window (appropriate for weekly data)
  bootstrap = TRUE,
...
# Basic Rt plot
rt_basic <- ggplot(rt_scabies_df %>% filter(!is.na(R_corrected)), aes(x = date)) +
...
rt_seasonal <- ggplot() +
  ...
  geom_line(
    data = rt_plot_data,
    aes(x = date, y = rt_smooth),
    linewidth = 1,
    color = "darkblue"
  ) +
  geom_ribbon(
    data = rt_plot_data,
    aes(x = date, ymin = lower_smooth, ymax = upper_smooth),
    alpha = 0.2,
    fill = "darkblue"
  ) +
