...
In this article, we will demonstrate how to use `mitey` to estimate $R^c_t$ using a synthetic data set, estimate $R^c_t$ using real data, and show several checks that can be used in practice when the true reproduction number is not known. We will also compare the estimates from `mitey` to estimates from other R packages that can be used for similar analyses, namely `EpiEstim` [@cori2013] and `EpiLPS` [@gressani2022].
