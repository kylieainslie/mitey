...
p <- ggplot(comparison_df %>%
  ...
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray50") +
  geom_ribbon(aes(ymin = wl_lower, ymax = wl_upper), fill = my_colors[3], alpha = 0.1) +
...
r_plot +
  geom_line(data = epifit$RLPS[-c(1:7),], aes(x = Time, y = R, color = "EpiLPS"), linewidth = 1, linetype = "solid") +
  geom_line(data = epifit$RWT, aes(x = t_end + zika2015$dates[1], y = `Mean(R)`, color = "WT (EpiEstim)"), linewidth = 1, linetype = "solid") +
...
ggplot(h7n9_complete, aes(x = date_onset, y = cases)) +
  ...
  labs(
    title = "Daily incidence of influenza A (H7N9) in China, 2013",
