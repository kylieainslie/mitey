...
line_plot <- ggplot(data = nivel_wkly_data %>%
                      mutate(time = seq(from = 1, to = nrow(nivel_wkly_data), by = 1)), 
...
# estimate Rt
# we will first approximate daily incidence using EpiEstim from our weekly aggregated
# incidence data
...
fancy_plot <- ggplot() + 
  geom_rect(data = rects, aes(xmin = xstart, xmax = xend, ymin = -Inf, ymax = Inf, fill = season), alpha = 0.4) +
  scale_fill_viridis_d() +
  geom_line(data = r_dat, aes(x = date, y = `Mean(R)`)) +
  #geom_ribbon(data = r_dat, aes(x = date, ymin = `Quantile.0.025(R)`, ymax = `Quantile.0.975(R)`)) +
